...
export const forgotPasswordSchema = z.object({
  email: z
    ...
    .refine(
      (email) => {
        const domain = email.toLowerCase().split("@")[1];
        return !["tempmail.com", "yopmail.com", "10minutemail.com"].includes(
          domain
        );
      },
...
const phoneValidation = z.string().regex(PHONE_REGEX, {
  message:
    "Le numéro de téléphone doit être au format international, commencer par un '+' suivi de l'indicatif du pays (1 à 3 chiffres), et contenir entre 9 et 12 chiffres.",
});

// Old +18 birthdate validation
const oldBirthDateValidation = z
  .string()
...
const parcelsList = z.array(
  z.object({
    height: z
      .number()
      .positive({ message: "Parcel height must be a positive number" }),
    width: z
      .number()
      .positive({ message: "Parcel width must be a positive number" }),
    length: z
      .number()
      .positive({ message: "Parcel length must be a positive number" }),
    weight: z
      .number()
      .positive({ message: "Parcel weight must be a positive number" }),
  })
);
// endregion
...
export const registerUserFrontendSchema = z
  .object({
...
// Lighter schema for admin-created users (no password fields)
export const registerUserFrontendSchemaAdmin = z
  .object({
    firstName: z.string().min(1, { message: "Le prénom est requis" }),
    lastName: z.string().min(1, { message: "Le nom est requis" }),
    birthDate: oldBirthDateValidation,
    ...
    email: emailValidation,
    address: addressSchema,
    // Optionally used for some roles
    agencyId: z
      .number()
      .int({ message: "L'identifiant de l'agence doit être un entier" })
      .positive({ message: "L'agence sélectionnée est invalide" })
...
export type RegisterUserFrontendAdminFormType = z.infer<typeof registerUserFrontendSchemaAdmin>;
...
export const tarifsSchema = z.object({
  ...
  volumeRate: z
    ...
    .default(0),
  baseRate: z.number({ message: "Base rate is required" }).default(0),
  fixedRate: z.number({ message: "Fixed rate is required" }).default(0),
});
...
export const transportSchema = z.object({
  ...
  baseVolume: z
    ...
    .max(MAX_TRANSPORT_VOLUME, {
      message: `Base volume cannot exceed ${MAX_TRANSPORT_VOLUME} cm³`,
    }),
  baseWeight: z
    .number()
    .min(1, { message: "The base weight of the vehicle is required" })
    .max(MAX_TRANSPORT_WEIGHT, {
      message: `Base weight cannot exceed ${MAX_TRANSPORT_WEIGHT} kg`,
    }),
  currentVolume: z
    .number()
    .min(0, { message: "The current capacity cannot be negative" })
    .max(MAX_TRANSPORT_VOLUME, {
      message: `Current volume cannot exceed ${MAX_TRANSPORT_VOLUME} cm³`,
    }),
  currentWeight: z
    .number()
    .min(0, { message: "The current weight cannot be negative" })
    .max(MAX_TRANSPORT_WEIGHT, {
      message: `Current weight cannot exceed ${MAX_TRANSPORT_WEIGHT} kg`,
    }),
  isAvailable: z.boolean().default(true),
});

export type RegisterUserFrontendFormType = z.infer<
  typeof registerUserFrontendSchema
>;
