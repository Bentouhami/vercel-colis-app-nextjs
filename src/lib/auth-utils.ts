...
export async function checkAuthStatus(showToast = true): Promise<AuthCheckResult> {
    try {
...
export async function requireAdmin() {
    return requireRole([RoleDto.SUPER_ADMIN, RoleDto.AGENCY_ADMIN, RoleDto.ACCOUNTANT])
}
...
export function isAdminRole(role?: RoleDto | null): boolean {
    ...
    return [RoleDto.SUPER_ADMIN, RoleDto.AGENCY_ADMIN, RoleDto.ACCOUNTANT].includes(role)
}
