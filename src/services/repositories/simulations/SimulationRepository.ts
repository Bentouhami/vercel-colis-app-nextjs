...
export class SimulationRepository implements ISimulationRepository {
  /**
     ...
     * @param envoiId
     */
  async getSimulationResponseById(
    envoiId: number
  ...
  async createSimulation(
    ...
    try {
      // Check if this block of code is reached
      console.log(
        "Creating simulation with data in SimulationRepository :",
        simulationData
      );
   ...
   */
  async updateSimulationUserId(
    simulationId: number,
    userId: number
  ): Promise<void | null> {
    const response = await simulationDAO.updateSimulationUserId(
      simulationId,
      userId
    );
   ...
   */
  async updateSimulationDestinataireId(
    simulationId: number,
    ...
    const response = await simulationDAO.updateSimulationDestinataireId(
      simulationId,
      destinataireId
    );
    if (!response) {
    ...
    try {
      ...
      const client: UserDto = {
        ...
        name: simulation.client?.name
          ? `${simulation.client?.lastName} ${simulation.client?.firstName}`
          : "",
        ...
        verificationTokenExpires:
          simulation.client?.verificationTokenExpires ?? new Date(),
        userAddresses: clientAddresses,
      };

      // prepare clientAddress object to return as AddressDto
      const destinataireAddresses =
        simulation.destinataire?.userAddresses ?? [];
      ...
      const destinataire: UserDto = {
        ...
        verificationTokenExpires:
          simulation.destinataire?.verificationTokenExpires ?? new Date(),
  ...
  async updateSimulationTransportId(
    simulationId: number,
    transportId: number
  ): Promise<boolean> {
    ...
    const response = await simulationDAO.updateSimulationTransportId(
      simulationId,
      transportId
    );
