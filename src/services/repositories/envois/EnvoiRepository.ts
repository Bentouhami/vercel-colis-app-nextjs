...
class EnvoiRepository implements IEnvoiRepository {
  ...
  async getEnvoiById(envoiId: number): Promise<EnvoiDto | null> {
    const envoi =
      await simulationRepository.getSimulationWithParcelsById(envoiId);
  ...
  async updateEnvoi(id: number, data: any): Promise<EnvoiDto | null> {
    ...
    try {
      ...
      const envoiStatus = envoi.envoiStatus as EnvoiStatusDto;

      // prepare simulationStatus object to return as SimulationStatusDto
      const simulationStatus = envoi.simulationStatus as SimulationStatusDto;
      ...
      const client: UserDto = {
        ...
        name: envoi.client?.name
          ? `${envoi.client?.lastName} ${envoi.client?.firstName}`
          : "",
        ...
        verificationTokenExpires:
          envoi.client?.verificationTokenExpires ?? new Date(),
      ...
      const desinataire: UserDto = {
        ...
        verificationTokenExpires:
          envoi.destinataire?.verificationTokenExpires ?? new Date(),
      ...
      const simulationObj: EnvoiDto = {
        ...
        envoiStatus: envoi.envoiStatus as EnvoiStatusDto,
        simulationStatus: envoi.simulationStatus as SimulationStatusDto,
        paid: envoi.paid,
        userId: envoi.userId!,
   ...
   * @param id
   */
  async getPaymentSuccessDataById(
    id: number
